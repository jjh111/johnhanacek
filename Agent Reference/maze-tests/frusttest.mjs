...
const r = await page.evaluate(async () => {
  ...
  draw(rect);
  const pen = recognizedShapes.find(s => s.type === 'rectangle');
  const fishInsideAtStart = F.state.fish.filter(f => pointInPoly(f.x, f.y, pen.idealPoints)).length;
  // food dead center of the empty pen
  draw([{x:1100,y:690},{x:1101.5,y:691}]);
  const fd = F.state.food[F.state.food.length - 1];
  const labelAtTap = whisperLabels.length ? whisperLabels[whisperLabels.length - 1].text : "none";
