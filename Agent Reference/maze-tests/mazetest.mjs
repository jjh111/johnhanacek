...
const r = await page.evaluate(async () => {
  ...
  drawPts(sq);
  out.square = { shapes: D.shapes.map(s => s.type), obstacles: externals() };
  ...
  out.food = { before: foodBefore, after: F.state.food.length, dotShapes: D.shapes.filter(s=>s.type==='dot').length };
  ...
  drawPts(sqg);
  out.squiggle = { shapesLeft: D.shapes.map(s=>s.type), obstacles: externals(), fishSurvived: F.state.fish.length };
  ...
  drawPts(sq2);
  const preClear = { shapes: D.shapes.length, obstacles: externals() };
  document.getElementById('clearCanvas').click();
  await new Promise(res => setTimeout(res, 100));
  out.clear = { preShapes: preClear.shapes, preObstacles: preClear.obstacles, postShapes: D.shapes.length, postObstacles: externals(), fishSurvived: F.state.fish.length };
  return out;
});
await page.waitForTimeout(400);
await page.screenshot({ path: (process.env.SHOT_DIR || '/tmp') + '/maze-shot.png', clip: { x: 0, y: 0, width: 1280, height: 800 } });
