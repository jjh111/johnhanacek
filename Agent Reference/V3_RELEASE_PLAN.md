...
- **Tweak round, 2026-09-11 (John's review):** the Clients/Teams strip moved up under the intro card and the intro's book/email row was removed; the design track's outcomes became **Client work** — OpenProse (Dan Barrett inline), Transfyr (project line; no Transfyr endorsement exists in the record), Muse.bio (Tommy Kronmark inline) — with Nanome and BadVR kept as **Past experience**, explicitly full-time roles rather than client engagements; the two options got "Option one/two" framing; `#fit` was reduced to Good fit / Not a fit, its "How it starts" copy moved to each track's CTA and its "Typical shape" copy to each track's engagement options. The two inline quotes left the testimonial wall (each voice once). Servicetest extended: inline-quote visibility, client/past block counts, fit card count, wall exclusion.
...
| Live-deploy verification (no CI) | agent | **BUILT 2026-09-10.** `scripts/check-live.mjs` — every sitemap page 200 + `?v=` coherence against `SITE.version`, resume PDF magic-byte check, both JSON artifacts parse; `--base` runs the same rules against a local server. Verified ALL PASS live, exit 1 against a dead base. Run after every push. |
