...
`maze-tests/`: a real headless Chromium and a `node_modules` containing
`playwright-core` (ESM ignores NODE_PATH; from the repo root, package.json pins
it). The suites resolve the browser as `CHROMIUM_PATH || chromium.executablePath()`
— no export needed, the old chromium-1217 path is dead.

```bash
# once, from the repo root
npm install
npm run browsers          # the PINNED installer

# serve the repo — the suites hardcode 127.0.0.1:4571
npm run test:serve        # or: python3 -m http.server 4571
