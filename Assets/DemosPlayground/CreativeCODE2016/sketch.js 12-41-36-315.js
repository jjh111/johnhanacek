...
//REPLACE SYNTH ELEMENTS WITH AMPLITUDE MODULATION EXAMPLE - REMOVE AND START OVER.
...
function setup() {
  frameRate(24);
  cnv = createCanvas(1500, 883, WEBGL);
  noCursor();
...
function draw() {
  ...
  var modFreq = map(mouseY, width / 2, 0, modMinFreq, modMaxFreq);
  ...
  var modDepth = map(mouseX, 0, height / 2, 0, modMinDepth, modMaxDepth);
  ...
  if (mouseIsPressed == true) {
    ...
    for (var i = 0; i < waveform.length; i++) {
      ...
      var y = map(waveform[i], -2, 2, -height / 2, height / 2);
      vertex(x, y + height / 2);
    }
    endShape();
    */
...
function explosion() {
  ...
  fill(244, 233, 202, 60);
  ellipse(mouseX, mouseY, radius * waveform.length, radius * waveform.height);
  //ellipse(mouseX, mouseY, radius * sin(radius++), radius * sin(radius++));
  //ellipse(mouseX, mouseY, radius * sin(waveform.length), radius * sin(waveform.length));

  //fill(172, 227, 219, alphaVal - 70);
  fill(172, 227, 219, 70);
  ellipse(mouseX, mouseY, waveform.height, waveform.length);
  //ellipse(mouseX, mouseY, radius * 2 * cos(radius++), radius * 2 * cos(radius++));
  //ellipse(mouseX, mouseY, radius * 2 * sin(waveform.length++), radius * 2 * cos(waveform.length++));
