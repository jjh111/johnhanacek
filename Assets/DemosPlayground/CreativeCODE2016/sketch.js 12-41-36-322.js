...
function setup() {
  ...
  function draw() {
    ...
    var locY = (touchY / height - 0.5) * (-2);
    var locX = (touchX / width - 0.5) * 2;


    // map mouseY to modulator freq between a maximum and minimum frequency
    var modFreq = map(touchY, height / 2, 0, modMinFreq, modMaxFreq); //-9
    ...
    // 
    var modDepth = map(touchX, 0, width / 2, modMinDepth, modMaxDepth); //84
  ...
  function explosion() {
    ...
    ellipse(touchX, touchY, radius * sin(radius++), radius * sin(radius++));

    fill(172, 227, 219, alphaVal - 70);
    ellipse(touchX, touchY, radius * 2 * cos(radius++), radius * 2 * cos(radius++));

    fill(102, 45, 145, alphaVal - 200);
    ellipse(touchX, touchY, radius * 8, radius * 8);
